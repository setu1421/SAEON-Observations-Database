    ...
    public class Startup
    {
        public void Configuration(IAppBuilder app)
        {
            using (SAEONLogs.MethodCall(GetType()))
            {
                try
                {
                    SAEONLogs.Verbose("IdentityServer: {name} RequireHttps: {RequireHTTPS}", Properties.Settings.Default.IdentityServerUrl, Properties.Settings.Default.RequireHTTPS);
                    ...
                    app.UseOpenIdConnectAuthentication(new OpenIdConnectAuthenticationOptions
                    {
                        ...
                        Notifications = new OpenIdConnectAuthenticationNotifications
                        {
                            ...
                            {
                                SAEONLogs.Verbose("AuthorizationCodeReceived notification");
                                var identity = new ClaimsIdentity(n.AuthenticationTicket.Identity.AuthenticationType);
                                SAEONLogs.Verbose("IsAuthenticated: {IsAuthenticated} ", identity.IsAuthenticated);
                                ...
                                {
                                    SAEONLogs.Error("UserInfo Error: {error}", userInfoResponse.Error);
                                    throw new HttpException(userInfoResponse.Error);

                                }
                                identity.AddClaims(userInfoResponse.Claims);

                                SAEONLogs.Verbose("Code: {Code} RedirectURI: {RedirectURI}", n.Code, n.RedirectUri);
                            ...
                            RedirectToIdentityProvider = n =>
                                {
                                    SAEONLogs.Verbose("RedirectToIdentityProvider notification");
