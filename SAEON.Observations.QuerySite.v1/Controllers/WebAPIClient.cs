...
namespace SAEON.Observations.QuerySite.Controllers
{
    public static class WebAPIClient
    {
        ...
        public static async Task<HttpClient> GetWebAPIClientAsync(HttpSessionStateBase session, ClaimsPrincipal user, bool isLocal)
        {
            ...
            SAEONLogs.Verbose("IsAuthenticated: {IsAuthenticated} Claims: {claims}", user?.Identity.IsAuthenticated, string.Join("; ", user.GetClaims()));
            ...
            SAEONLogs.Verbose("ClaimsPrincipal: IdToken: {IdToken} AccessToken: {AccessToken} RefreshToken: {RefreshToken} ExpiresAt: {ExpiresAt}", idToken, accessToken, refreshToken, expiresAt);
            if (accessToken == null)
            {
                SAEONLogs.Verbose("Getting AccessToken");
                using (var discoClient = new HttpClient())
                {
                    ...
                    if (disco.IsError)
                    {
                        SAEONLogs.Error("Disco Error: {error}", disco.Error);
                    ...
                    using (var tokenClient = new HttpClient())
                    {
                        ...
                        {
                            SAEONLogs.Error("Token Error: {error}", tokenResponse.Error);
            ...
            //    {
            //        SAEONLogs.Verbose("Refreshing Tokens");
            ...
            //    }
            //}
            SAEONLogs.Verbose("IdToken: {IdToken} AccessToken: {AccessToken} RefreshToken: {RefreshToken} ExpiresAt: {ExpiresAt}", idToken, accessToken, refreshToken, expiresAt);
