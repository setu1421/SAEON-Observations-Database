...
Update
  Sensor
set
  StationID = (Select ID from Station where Code = 'CONSTB_CWS_W')
from
  Sensor
  inner join Station
    on (Sensor.StationID = Station.ID)
where
  (Station.Code = 'Const_test')
Update
  Sensor
set
  DataSourceID = (Select ID from DataSource where Code = 'CONSTB_CWS_CSAWS')
from
  Sensor
  inner join DataSource
    on (Sensor.DataSourceID = DataSource.ID)
where
  (DataSource.Code = 'Const_test')
Update
  DataSchema
Set
  Code = 'DAV_PR2'
...
where
  (Code = 'CONSTB_CWS_CSAWS_SM6_D')   
Update 
  Sensor
set
  DataSourceID = (Select ID from DataSource where Code = 'CONSTB_CWS_CSAWS_D')
where
  (Code = 'CONSTB_CWS_CSAWS_T_D')   
Update 
  Sensor
set
  DataSourceID = (Select ID from DataSource where Code = 'CONSTB_CWS_CSAWS_D')
where
  (Code = 'CONSTB_CWS_CSAWS_WD_D')
Update 
  Sensor
set
  DataSourceID = (Select ID from DataSource where Code = 'CONSTB_CWS_CSAWS_D')
where
  (Code = 'CONSTB_CWS_CSAWS_WS_D')
Update 
  Sensor
set
  DataSourceID = (Select ID from DataSource where Code = 'CONSTB_CWS_CSAWS_D')
