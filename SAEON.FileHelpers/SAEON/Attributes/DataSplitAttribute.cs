    ...
    public sealed class DataSplitAttribute : Attribute
    {
        ...
        public DataSplitAttribute(string splitSelector, int splitIndex)
        {
            if (String.IsNullOrEmpty(splitSelector))
                throw new BadUsageException("The split Selector value cannot be empty");
            else
                this.SplitSelector = splitSelector;


            if (SplitIndex <= 0)
                throw new BadUsageException("The split Index of the DataSplit Attribute must be greater than 0");
            else
