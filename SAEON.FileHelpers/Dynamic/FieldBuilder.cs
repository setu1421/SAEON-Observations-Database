...
namespace FileHelpers.Dynamic
{
    ...
    public abstract partial class FieldBuilder
    {
        ...
        private void AddAttributesInternal(AttributesBuilder attbs, NetLanguage lang)
        {
            // TimPN
            if (mValueDate == true)
                attbs.AddAttribute(String.Concat("ValueDate(\"", this.mValueDateformat, "\")"));

            if (mValueTime == true)
                attbs.AddAttribute(String.Concat("ValueTime(\"", this.mValueTimeformat, "\")"));

            if (mValueEmpty == true)
                attbs.AddAttribute(String.Concat("ValueEmpty(\"", this.mEmptyValue, "\")"));

            if (mValueComment == true)
                attbs.AddAttribute(String.Concat("ValueComment()"));

            if (mphenomenonOfferingID.HasValue && !mphenomenonOfferingID.Equals(Guid.Empty))
                attbs.AddAttribute(String.Concat("PhenomenonOffering(\"", mphenomenonOfferingID, "\")"));

            if (mphenomenonUOMID.HasValue && !mphenomenonUOMID.Equals(Guid.Empty))
                attbs.AddAttribute(String.Concat("PhenomenonUOM(\"", mphenomenonUOMID, "\")"));

            if (!String.IsNullOrEmpty(this.mFixedTime))
                attbs.AddAttribute(String.Concat("FixedTimeAttribute(\"", this.mFixedTime, "\")"));
            // TimPN
        ...
        internal void SaveToXml(XmlHelper writer)
        {
            ...
            // TimPN
            writer.WriteElement("ValueDate", this.ValueDate);
            if (this.ValueDate)
                writer.WriteElement("ValueDateformat", this.ValueDateformat);

            writer.WriteElement("ValueTime", this.ValueTime);

            if (this.ValueTime)
                writer.WriteElement("ValueTimeformat", this.ValueTimeformat);

            writer.WriteElement("ValueEmpty", this.ValueEmpty);
            if (this.ValueEmpty)
                writer.WriteElement("EmptyValue", this.EmptyValue);


            writer.WriteElement("ValueComment", this.ValueComment);

            if (PhenomenonOfferingID.HasValue && !PhenomenonOfferingID.Equals(Guid.Empty))
                writer.WriteElement("PhenomenonOfferingID", this.PhenomenonOfferingID.ToString());

            if (PhenomenonUOMID.HasValue && !PhenomenonUOMID.Equals(Guid.Empty))
                writer.WriteElement("PhenomenonUOMID", this.PhenomenonUOMID.ToString());

            if (!String.IsNullOrEmpty(this.FixedTime))
                writer.WriteElement("FixedTime", this.FixedTime);
            // TimPN
        ...
        internal void ReadField(XmlNode node)
        {
            ...
            // TimPN
            ValueDate = node["ValueDate"] != null;

            ele = node["ValueDateformat"];
            if (ele != null)
                ValueDateformat = ele.InnerText;

            ValueTime = node["ValueTime"] != null;

            ele = node["ValueTimeformat"];
            if (ele != null)
                ValueTimeformat = ele.InnerText;

            ValueEmpty = node["ValueEmpty"] != null;

            ele = node["EmptyValue"];
            if (ele != null)
                EmptyValue = ele.InnerText;


            ValueComment = node["ValueComment"] != null;

            ele = node["PhenomenonOfferingID"];
            if (ele != null)
                PhenomenonOfferingID = Guid.Parse(ele.InnerText);

            ele = node["PhenomenonUOMID"];
            if (ele != null)
                PhenomenonUOMID = Guid.Parse(ele.InnerText);


            ele = node["FixedTime"];
            if (ele != null)
                FixedTime = ele.InnerText;
