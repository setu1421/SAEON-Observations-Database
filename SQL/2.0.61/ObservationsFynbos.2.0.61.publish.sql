...
--    CONSTRAINT [tmp_ms_xx_constraint_UX_Observation1] UNIQUE NONCLUSTERED ([SensorID] ASC, [ValueDate] ASC, [DataValue] ASC, [PhenomenonOfferingID] ASC, [PhenomenonUOMID] ASC, [Elevation] ASC) ON [Observations]
--);
...
IF EXISTS (SELECT TOP 1 1 
    ...
    BEGIN
        INSERT INTO [dbo].[tmp_ms_xx_UserDownloads] ([ID], [UserId], [Name], [Description], [AddedAt], [AddedBy], [UpdatedAt], [UpdatedBy], [MetadataURL], [DownloadURL], [Citation], [Title], [Keywords], [Date], [Input], [RequeryURL], [DigitalObjectIdentifierID], [MetadataJson], [ZipFullName], [ZipCheckSum], [ZipURL], [Places], [LatitudeNorth], [LatitudeSouth], [LongitudeWest], [LongitudeEast], [ElevationMinimum], [ElevationMaximum], [StartDate], [EndDate], [OpenDataPlatformID])
        SELECT   [ID],
                 [UserId],
                 [Name],
                 [Description],
