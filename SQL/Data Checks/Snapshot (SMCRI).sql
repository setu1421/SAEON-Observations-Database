...
from 
	vDatasetsExpansion
where
  (IsValid = 1) and (StationCode like 'ELW_%')
	--(VerifiedCount > 0)  and 
...
from
	vImportBatchSummary
where
    (OrganisationCode in ('SAEON','SMCRI','EFTEON')) and (ProgrammeCode <> 'SACTN') and (StationCode like 'ELW_%') and 
	(VerifiedCount > 0)  and 
	(LatitudeNorth is not null) and (LatitudeSouth is not null) and 
	(LongitudeEast is not null) and (LongitudeWest is not null)
)
Select 
	...
	(Select Count(*) from vVariables where (PhenomenonName like 'EOV_%')) Variables,
